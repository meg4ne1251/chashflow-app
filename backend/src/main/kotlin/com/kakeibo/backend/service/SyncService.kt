...
class SyncService(
    ...
    fun pull(since: String): SyncPullResponse {
        ...
        val hasMore = transactions.size >= PULL_LIMIT || categories.size >= PULL_LIMIT ||
                ...
                templates.size >= PULL_LIMIT || recurringTransactions.size >= PULL_LIMIT ||
                budgets.size >= PULL_LIMIT || transfers.size >= PULL_LIMIT ||
                notificationSettings.size >= PULL_LIMIT || inputPatterns.size >= PULL_LIMIT
